...
welcome() {
  ...
  print_brake 70
  output "Pterodactyl installation script @ $SCRIPT_RELEASE"
  output ""
  output "Customized by Ayanok0ji (https://github.com/Ayanok0ji/pterodactyl-installer)"
  output "CCTO / Credits to original author Vilhelm Prytz & contributors"
  ...
  if [ "$1" == "panel" ]; then
    output "Latest pterodactyl/panel release is $PTERODACTYL_PANEL_VERSION"
  elif [ "$1" == "wings" ]; then
    output "Latest pterodactyl/wings release is $PTERODACTYL_WINGS_VERSION"
  fi
  print_brake 70
}
...
format_version() {
  local ver="$1"
  if [[ "$ver" == "latest" || "$ver" == "LATEST" || -z "$ver" ]]; then
    echo "latest"
  elif [[ "$ver" =~ ^[0-9] ]]; then
    echo "v$ver"
  else
...
verify_release_url() {
  local url="$1"
  local status_code
  status_code=$(curl -sIL -o /dev/null -w "%{http_code}" "$url" 2>/dev/null || echo "000")
...
set_panel_version() {
  ...
  if [ "$ver" == "latest" ]; then
    local latest_tag
    latest_tag=$(get_latest_release "pterodactyl/panel")
    export PTERODACTYL_PANEL_VERSION="${latest_tag:-latest}"
    export PANEL_DL_URL="https://github.com/pterodactyl/panel/releases/latest/download/panel.tar.gz"
  else
...
set_wings_version() {
  ...
  if [ "$ver" == "latest" ]; then
    local latest_tag
    latest_tag=$(get_latest_release "pterodactyl/wings")
    export PTERODACTYL_WINGS_VERSION="${latest_tag:-latest}"
    export WINGS_DL_BASE_URL="https://github.com/pterodactyl/wings/releases/latest/download/wings_linux_"
  else
    export PTERODACTYL_WINGS_VERSION="$ver"
    export WINGS_DL_BASE_URL="https://github.com/pterodactyl/wings/releases/download/${PTERODACTYL_WINGS_VERSION}/wings_linux_"
  fi
}

get_recent_releases() {
  curl -sL "https://api.github.com/repos/$1/releases?per_page=6" 2>/dev/null |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/' |
    tr '\n' ' '
}
...
get_latest_release() {
  curl -sL "https://api.github.com/repos/$1/releases/latest" 2>/dev/null | # Get latest release from GitHub api
...
get_latest_versions() {
  ...
  local panel_ver wings_ver
  panel_ver=$(get_latest_release "pterodactyl/panel")
  wings_ver=$(get_latest_release "pterodactyl/wings")
  PTERODACTYL_PANEL_VERSION="${panel_ver:-latest}"
  PTERODACTYL_WINGS_VERSION="${wings_ver:-latest}"
}
...
create_db_user() {
  ...
  mariadb -u root -e "CREATE USER IF NOT EXISTS '$db_user_name'@'$db_host' IDENTIFIED BY '$db_user_password';" 2>/dev/null || mariadb -u root -e "CREATE USER '$db_user_name'@'$db_host' IDENTIFIED BY '$db_user_password';"
  mariadb -u root -e "ALTER USER '$db_user_name'@'$db_host' IDENTIFIED BY '$db_user_password';" 2>/dev/null || true
